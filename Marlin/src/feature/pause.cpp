...
static bool ensure_safe_temperature(const PauseMode mode=PAUSE_MODE_SAME) {
    ...
    if (!DEBUGGING(DRYRUN) && thermalManager.targetTooColdToExtrude(active_extruder)) {
      SERIAL_ECHO_MSG(MSG_HOTEND_TOO_COLD);
      return false;
    }
  #endif

  #if HAS_LCD_MENU
    lcd_advanced_pause_show_message(PAUSE_MESSAGE_HEATING, mode);
  #else
    UNUSED(mode);
  #endif
...
bool load_filament(const float &slow_load_length/*=0*/, const float &fast_load_length/*=0*/, const float &purge_length/*=0*/, const int8_t max_beep_count/*=0*/,
  ...
  if (!ensure_safe_temperature(mode)) {
    #if HAS_LCD_MENU
      if (show_lcd) lcd_advanced_pause_show_message(PAUSE_MESSAGE_STATUS, mode);
    #endif
    return false;
  }
  ...
  if (pause_for_user) {
    #if HAS_LCD_MENU
      if (show_lcd) lcd_advanced_pause_show_message(PAUSE_MESSAGE_INSERT, mode);
    #endif
    SERIAL_ECHO_MSG(MSG_FILAMENT_CHANGE_INSERT_LCD);
  ...
  #if HAS_LCD_MENU
    if (show_lcd) lcd_advanced_pause_show_message(PAUSE_MESSAGE_LOAD, mode);
  #endif
  ...
  if (fast_load_length) {
    ...
    #if HAS_LCD_MENU
      if (show_lcd) lcd_advanced_pause_show_message(PAUSE_MESSAGE_PURGE);
    #endif
    ...
    do {
      if (purge_length > 0) {
        ...
        if (show_lcd) {
          ...
          KEEPALIVE_STATE(PAUSED_FOR_USER);
          wait_for_user = false;
          lcd_advanced_pause_show_message(PAUSE_MESSAGE_OPTION);
...
bool unload_filament(const float &unload_length, const bool show_lcd/*=false*/,
  ...
  if (!ensure_safe_temperature(mode)) {
    #if HAS_LCD_MENU
      if (show_lcd) lcd_advanced_pause_show_message(PAUSE_MESSAGE_STATUS);
    #endif
    return false;
  }

  #if HAS_LCD_MENU
    if (show_lcd) lcd_advanced_pause_show_message(PAUSE_MESSAGE_UNLOAD, mode);
  #endif
...
void show_continue_prompt(const bool is_reload) {
  #if HAS_LCD_MENU
    lcd_advanced_pause_show_message(is_reload ? PAUSE_MESSAGE_INSERT : PAUSE_MESSAGE_WAITING);
  #endif
  SERIAL_ECHO_START();
  serialprintPGM(is_reload ? PSTR(MSG_FILAMENT_CHANGE_INSERT_LCD "\n") : PSTR(MSG_FILAMENT_CHANGE_WAIT_LCD "\n"));
}
...
void wait_for_confirmation(const bool is_reload/*=false*/, const int8_t max_beep_count/*=0*/ DXC_ARGS) {
  ...
  while (wait_for_user) {
    ...
    if (nozzle_timed_out) {
      #if HAS_LCD_MENU
        lcd_advanced_pause_show_message(PAUSE_MESSAGE_HEAT);
      #endif
      SERIAL_ECHO_MSG(MSG_FILAMENT_CHANGE_HEAT_LCD);
...
void resume_print(const float &slow_load_length/*=0*/, const float &fast_load_length/*=0*/, const float &purge_length/*=ADVANCED_PAUSE_PURGE_LENGTH*/, const int8_t max_beep_count/*=0*/ DXC_ARGS) {
  ...
  if (nozzle_timed_out || thermalManager.hotEnoughToExtrude(active_extruder)) // Load the new filament
    load_filament(slow_load_length, fast_load_length, purge_length, max_beep_count, false, false, PAUSE_MODE_SAME DXC_PASS);

  #if HAS_LCD_MENU
    lcd_advanced_pause_show_message(PAUSE_MESSAGE_RESUME);
  #endif
