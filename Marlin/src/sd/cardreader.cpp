...
uint8_t CardReader::longlongFilename[LONG_FILENAME_LENGTH * 2];
uint8_t CardReader::utf8_filename[60];
...
int CardReader::countItems(SdFile dir)
{
  dir_t p;
  int c = 0;
  while (dir.readDir(&p, longFilename, longlongFilename) > 0)
    c += is_dir_or_gcode(p);
...
{
  dir_t p;
  for (uint8_t cnt = 0; dir.readDir(&p, longFilename, longlongFilename) > 0;)
  {
...
{
  dir_t p;
  for (uint8_t cnt = 0; dir.readDir(&p, longFilename, longlongFilename) > 0; cnt++)
  {
    if (is_dir_or_gcode(p))
    {
      createFilename(filename, p);
...
{
  dir_t p;
  while (parent.readDir(&p, longFilename, longlongFilename) > 0)
  {
    if (DIR_IS_SUBDIR(&p))
    {
...
void CardReader::lsRoot(void)
{
  uint16_t _filecount = countFilesInWorkDir();
  char fileline[30];

  for (uint16_t i = 0; i < _filecount; i++)
  {
    card.getfilename_sorted(_filecount - i - 1);
    sprintf_P(fileline, PSTR("LN:%s S:%ld"), filename, filesize); // 发送一条文件信息
    wifi_manager.FIleListSend(fileline);
...
//
bool CardReader::openFileRead(char *const path, const uint8_t subcall_type /*=0*/)
{
  if (!isMounted())
    return false;
...
//
bool CardReader::openFileWrite(char *const path)
{
  if (!isMounted())
    return false;
...
//
int CardReader::removeFile(const char *const name)
{
  if (!isMounted())
    return -1;
...
void CardReader::write_command(char *const buf)
{
  ...
  if (file.writeError)
    SERIAL_ERROR_MSG(MSG_SD_ERR_WRITE_TO_FILE);
}

bool CardReader::write_line(char *const buf)
{
  char *begin = buf;
  char *end = buf + strlen(buf) - 1;
...
bool CardReader::openNewFileWrite(char *const path)
{
  if (!isMounted())
    return false;
  ...
  if (file.open(curDir, fname, O_CREAT | O_WRITE | O_TRUNC))
  {
    flag.saving = true;
    selectFileByName(fname);
    echo_write_to_file(fname);
...
bool CardReader::openSavFileRead(char *const path)
{
  if (!isMounted())
    return false;
...
void CardReader::checkautostart()
{
  ...
  if (autostart_index < 0 || flag.sdprinting)
    return;
  ...
  )
  {
    ...
    dir_t p;
    root.rewind();
    while (root.readDir(&p, nullptr, nullptr) > 0)
    {
      for (int8_t i = (int8_t)strlen((char *)p.name); i--;)
...
#define SET_SORTNAME(I)                          \
  do                                             \
  {                                              \
    _SET_SORTNAME(I);                            \
    sortnames[I][SORTED_LONGNAME_MAXLEN] = '\0'; \
  } while (0)
#endif
#if ENABLED(SDSORT_CACHE_NAMES)
...
void CardReader::removeJobRecoveryFile()
{
  ...
  if (_exist)
  {
    ...
    PORT_REDIRECT(0);
    SERIAL_ECHOLNPGM("Power-loss file delete.");
    PORT_RESTORE();
...
{
  recovery.file.seekSet(0);
  const int16_t ret = recovery.file.write(&recovery.info, sizeof(recovery.info));
#if ENABLED(DEBUG_POWER_LOSS_RECOVERY)
  if (ret == -1)
    SERIAL_PROTOCOLLNPGM("Power-loss file write failed.");
#endif
  recovery.file.close();
  return ret;
}
...
uint32_t CardReader::checkFileExist(const char *name)
{
  SdFile *curDir;
...
void CardReader::getPrintingFilename(char *filename)
{
  if (file.isOpen())
  {
    file.getDosName(filename);
  }
}

int CardReader::getPrintPercent()
{
  if (flag.mounted && flag.sdprinting && (filesize > 0))
  {
    return (sdpos * 100 / filesize);
  }
  else
  {
    return 0;
...
void CardReader::get_utf8_filename(void)
{
  longFilename[26] = '\0';
  ZERO(utf8_filename);
  if (longFilename[0] != 0)
  {
    uint16_t _len = strlen(longFilename) * 2;
    UnicodeToUtf8((uint8_t *)longlongFilename, utf8_filename, _len, 60);
  }
  else
  {
    strcpy((char *)utf8_filename, filename);
  }
}
