...
bool deleteRecoveryFile = false;
bool wtvar_uart0_binmode = false;
bool wtvar_click_cancel_heat = false;
bool wtvar_wait_emove = false;
bool wtvar_click_cancel_emove = false;
// static bool ready_Send_Empty = false;
bool printoverinfoscreenflag;
uint8_t wtvar_enablepowerloss = 0;
float zoffset_forjust = 0.0;
uint32_t nowtime;
uint32_t posttime;
bool readykill = false;
int progressvalue;
const char* killmsg = NULL;
// static bool door_open_out = false;
uint8_t wtvar_status_time = 1;		// status screen time type: 0 - printjob time, 1 - printing time used, 2 - printing time left
uint8_t wtvar_quiet_mode = 0;
uint8_t wtvar_led_state = 1;

uint16_t wtvar_fps = 0;

extern void wt_restart();
extern void lcd_job_recovery_menu();
...
void kill(PGM_P const lcd_error/*=nullptr*/, PGM_P const lcd_component/*=nullptr*/, const bool steppers_off/*=false*/) {
  ...
  wifi_manager.ShowErrorMessage(lcd_error);
...
void setup() {
  ...
  if (IS_SD_INSERTED()) card.mount();

  SETUP_RUN(lcd_init());
  SETUP_RUN(lcd_reset_status());       // Load welcome message early. (Retained if no errors exist.)
  SETUP_RUN(lcd_bootscreen());
  ...
  #if ENABLED(SDSUPPORT)
    SETUP_RUN(wt_load_sd());          // Mount the SD card before settings.first_load
  #endif
  ...
  // show welcome screen
	...
	{
		queue.enqueue_one_P(PSTR("W100"));
		queue.enqueue_one_P(PSTR("G28 Z F300"));
		queue.enqueue_one_P(PSTR("M18"));
	}
	else if (wtvar_gohome == 1)
	{
		wtvar_gohome = 0;
		(void)settings.save();
		queue.enqueue_one_P(PSTR("G28 Z F300"));
