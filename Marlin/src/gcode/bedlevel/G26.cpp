...
int8_t g26_prime_flag;
  ...
  bool user_canceled() {
    ...
    lcd_setstatus_P(GET_TEXT(MSG_G26_CANCELED), 99);
    #if HAS_LCD_MENU
      ui.quick_feedback();
    #endif
    ui.wait_for_release();
...
inline bool turn_on_heaters() {
  ...
  SERIAL_ECHOLNPGM("Waiting for heatup.");
    ...
    if (g26_bed_temp > 25) {
      #if HAS_SPI_LCD
        lcd_setstatus_P(GET_TEXT(MSG_G26_HEATING_BED), 99);
        ui.quick_feedback();
        #if HAS_LCD_MENU
          ui.capture();
        #endif
  ...
  #if HAS_SPI_LCD
    lcd_setstatus_P(GET_TEXT(MSG_G26_HEATING_NOZZLE), 99);
    ui.quick_feedback();
  #endif
...
inline bool prime_nozzle() {
  ...
  const feedRate_t fr_slow_e = planner.settings.max_feedrate_mm_s[E_AXIS] / 15.0f;
    ...
    if (g26_prime_flag == -1) {  // The user wants to control how much filament gets purged
      ui.capture();
      lcd_setstatus_P(GET_TEXT(MSG_G26_MANUAL_PRIME), 99);
      ui.chirp();
      ...
      lcd_setstatus_P(GET_TEXT(MSG_G26_PRIME_DONE), 99);
      ui.quick_feedback();
      ui.release();
    }
    else
  ...
  {
    #if HAS_SPI_LCD
      lcd_setstatus_P(GET_TEXT(MSG_G26_FIXED_LENGTH), 99);
      ui.quick_feedback();
    #endif
    destination = current_position;
...
void GcodeSuite::G26() {
  ...
  LEAVE:
  lcd_setstatus_P(GET_TEXT(MSG_G26_LEAVING), -1);
