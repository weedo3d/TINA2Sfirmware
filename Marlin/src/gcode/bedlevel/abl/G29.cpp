...
G29_TYPE GcodeSuite::G29() {
  ...
  if (!g29_in_progress) {
	...
	#ifdef SERIAL_PORT_2
      SERIAL_ECHOLNPGM_0("?(V)erbose level implausible (0-4).");
	#else
    ...
    if (verbose_level > 0) {
	#ifdef SERIAL_PORT_2
      SERIAL_ECHOPGM_0("G29 Auto Bed Leveling");
      if (dryrun) SERIAL_ECHOPGM_0(" (DRYRUN)");
	#else
    ...
    measured_z = 0;
      ...
      for (PR_OUTER_VAR = 0; PR_OUTER_VAR < PR_OUTER_END && !isnan(measured_z); PR_OUTER_VAR++) {
        ...
        for (PR_INNER_VAR = inStart; PR_INNER_VAR != inStop; pt_index++, PR_INNER_VAR += inInc) {
          ...
          #if HAS_DISPLAY
            lcd_status_printf_P(0, PSTR(S_FMT " %i/%i"), (MSG_PROBING_MESH), int(pt_index), int(GRID_MAX_POINTS));
          #endif
      ...
      LOOP_L_N(i, 3) {
	  #ifdef SERIAL_PORT_2
        if (verbose_level) SERIAL_ECHOLNPAIR_0("Probing point ", int(i), "/3.");
	  #else
        if (verbose_level) SERIAL_ECHOLNPAIR("Probing point ", int(i), "/3.");
	  #endif
        #if HAS_DISPLAY
          lcd_status_printf_P(0, PSTR(S_FMT " %i/3"), MMSG_PROBING_MESH[wtvar_language], int(i));
        #endif
