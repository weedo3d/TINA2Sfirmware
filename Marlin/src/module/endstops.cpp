...
void Endstops::not_homing() {
  ...
  void Endstops::validate_homing_move(const AxisEnum axis) {
    ...
    {
      if (axis == X_AXIS)
        kill(MMSG_ERR_X_HOMING_FAILED[wtvar_language]);
      else if (axis == Y_AXIS)
        kill(MMSG_ERR_Y_HOMING_FAILED[wtvar_language]);
      else
        kill(MMSG_ERR_Z_HOMING_FAILED[wtvar_language]);
    }
  }
#endif
...
void Endstops::event_handler() {
  ...
  if (hit_state) {
    ...
    #if HAS_SPI_LCD
      lcd_status_printf_P(0, PSTR(S_FMT " %c %c %c %c"), MSG_LCD_ENDSTOPS, chrX, chrY, chrZ, chrP);
    #endif
