...
extern void waitkill(const char* lcd_error);
...
const char MSG_t_thermal_runaway[] PROGMEM = MSG_T_THERMAL_RUNAWAY,
           MSG_t_heating_failed[] PROGMEM = MSG_T_HEATING_FAILED;
...
#endif
#define _E_PSTR(h,N) ((HOTENDS) > N && (h) == N) ? PSTR(LCD_MSG_E##N) :
#define HEATER_PSTR(h) _BED_PSTR(h) _CHAMBER_PSTR(h) _E_PSTR(h,1) _E_PSTR(h,2) _E_PSTR(h,3) _E_PSTR(h,4) _E_PSTR(h,5) PSTR(LCD_MSG_E0)
...
#define TEMPDIR(N) ((HEATER_##N##_RAW_LO_TEMP) < (HEATER_##N##_RAW_HI_TEMP) ? 1 : -1)
  ...
  bool Temperature::auto_fan_on = false;
#endif
  ...
  uint8_t Temperature::soft_pwm_amount_autofan[1];
  uint16_t Temperature::soft_pwm_count_autofan[1];
#endif
        ...
        if (!heating && current_temp < target) {
          if (ELAPSED(ms, t1 + 5000UL)) {
            ...
            if (cycles > 0) {
              ...
              SERIAL_ECHOPAIR(MSG_BIAS, bias, MSG_D_COLON, d, MSG_T_MIN, minT, MSG_T_MAX, maxT);
              if (cycles > 2) {
        ...
        // Make sure heating is actually working
          ...
          if (
            ...
            if (!heated) {                                          // If not yet reached target...
              ...
              {
                if (heater == H_E0)
                  _temp_error(heater, MSG_t_heating_failed, MMSG_NOZZLE_HEATING_FAILED_LCD[wtvar_language]);
                else
                  _temp_error(heater, MSG_t_heating_failed, MMSG_BED_HEATING_FAILED_LCD[wtvar_language]);
              }
            }
            else if (current_temp < target - (MAX_OVERSHOOT_PID_AUTOTUNE)) // Heated, then temperature fell too far?
              // _temp_error(heater, MSG_t_thermal_runaway, MMSG_THERMAL_RUNAWAY[wtvar_language]);
              {
                if (heater == H_E0)
                  _temp_error(heater, MSG_t_heating_failed, MMSG_NOZZLE_HEATING_FAILED_LCD[wtvar_language]);
                else
...
void Temperature::max_temp_error(const heater_ind_t heater) {
  // _temp_error(heater, PSTR(MSG_T_MAXTEMP), MMSG_ERR_MAXTEMP[wtvar_language]);
  if (heater == H_E0)
    _temp_error(heater, PSTR(MSG_T_MAXTEMP), MMSG_ERR_NOZZLE_TEMP[wtvar_language]);
  else
    _temp_error(heater, PSTR(MSG_T_MAXTEMP), MMSG_ERR_BED_TEMP[wtvar_language]);
}
...
void Temperature::manage_heater() {
  ...
  millis_t ms = millis();
    ...
    HOTEND_LOOP() {
        ...
        {
          if (e == H_E0)
            _temp_error((heater_ind_t)e, MSG_t_thermal_runaway, MMSG_ERR_NOZZLE_TEMP[wtvar_language]);
          else
            _temp_error((heater_ind_t)e, MSG_t_thermal_runaway, MMSG_ERR_BED_TEMP[wtvar_language]);
        }
      ...
      temp_hotend[e].soft_pwm_amount = (temp_hotend[e].celsius > temp_range[e].mintemp || is_preheating(e)) && temp_hotend[e].celsius < temp_range[e].maxtemp ? (int)get_pid_output_hotend(e) >> 1 : 0;
        ...
        if (watch_hotend[e].next_ms && ELAPSED(ms, watch_hotend[e].next_ms)) {  // Time to check this extruder?
          if (degHotend(e) < watch_hotend[e].target)     
          {
            if (e == H_E0)
              _temp_error((heater_ind_t)e, MSG_t_heating_failed, MMSG_NOZZLE_HEATING_FAILED_LCD[wtvar_language]);
            else
              _temp_error((heater_ind_t)e, MSG_t_heating_failed, MMSG_BED_HEATING_FAILED_LCD[wtvar_language]);
        ...
        if (ABS(temp_hotend[0].celsius - redundant_temperature) > MAX_REDUNDANT_TEMP_SENSOR_DIFF)
          _temp_error(H_E0, PSTR(MSG_REDUNDANCY), MSG_ERR_REDUNDANT_TEMP);
      #endif
    ...
    filwidth.update_volumetric();
      ...
      if (degBed() > BED_MAXTEMP)
        _temp_error(H_BED, MSG_t_thermal_runaway, MMSG_ERR_BED_TEMP[wtvar_language]);
    #endif
      ...
      if (watch_bed.elapsed(ms)) {        // Time to check the bed?
        if (degBed() < watch_bed.target)                                // Failed to increase enough?
          _temp_error(H_BED, MSG_t_heating_failed, MMSG_BED_HEATING_FAILED_LCD[wtvar_language]);
        else                                                            // Start again if the target is still far off
          start_watching_bed();
      }
    #endif // WATCH_BED
    ...
    do {
      ...
      if (degChamber() > CHAMBER_MAXTEMP)
        _temp_error(H_CHAMBER, MSG_t_thermal_runaway, MMSG_THERMAL_RUNAWAY[wtvar_language]);
    #endif
      ...
      if (watch_chamber.elapsed(ms)) {              // Time to check the chamber?
        if (degChamber() < watch_chamber.target)    // Failed to increase enough?
          _temp_error(H_CHAMBER, MSG_t_heating_failed, MMSG_HEATING_FAILED_LCD[wtvar_language]);
        else
...
#define SCAN_THERMISTOR_TABLE(TBL,LEN) do{                             \
  ...
  float Temperature::analog_to_celsius_hotend(const int raw, const uint8_t e) {
    ...
    #endif
      {
        ...
        // kill();
        waitkill(MMSG_NOZZLE_HEATING_FAILED_LCD[wtvar_language]);
        return 0;
      }
...
void Temperature::init() {
  ...
  #if ENABLED(FAN_SOFT_PWM)
  #if HAS_AUTO_FAN_0
  soft_pwm_amount_fan[0] = AUTO_FAN_PWM_MAX;
  #endif
  #endif
}
  ...
  void Temperature::thermal_runaway_protection(Temperature::tr_state_machine_t &sm, const float &current, const float &target, const heater_ind_t heater_id, const uint16_t period_seconds, const uint16_t hysteresis_degc) {
    ...
    switch (sm.state) {
      ...
      {
        if (heater_id == H_E0)
          _temp_error(heater_id, MSG_t_thermal_runaway, MMSG_ERR_NOZZLE_TEMP[wtvar_language]);
        else
          _temp_error(heater_id, MSG_t_thermal_runaway, MMSG_ERR_BED_TEMP[wtvar_language]);
      }   
    }
  }
...
void Temperature::tick() {
  ...
  if (auto_fan_on)
  {
    if (soft_pwm_amount_autofan[0] == 255)
    {
      WRITE(E0_AUTO_FAN_PIN, 1);
    }
    else
    {
      if (soft_pwm_count_autofan[0] < soft_pwm_amount_autofan[0])
        WRITE(E0_AUTO_FAN_PIN, 1);
      else
        WRITE(E0_AUTO_FAN_PIN, 0);
    }
  }
  else
  {
  ...
  soft_pwm_count_autofan[0] += 20;
  if (soft_pwm_count_autofan[0] > 255) soft_pwm_count_autofan[0] = 0;
  #endif
  #endif 
  ...
  void Temperature::print_heater_states(const uint8_t target_extruder
    ...
    void Temperature::set_heating_message(const uint8_t e) {
      ...
      lcd_setstatusPGM(heating ? MMSG_HEATING[wtvar_language] : MMSG_COOLING[wtvar_language]);
    }
  #endif
