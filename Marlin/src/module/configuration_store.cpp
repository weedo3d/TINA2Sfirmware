...
#include "../core/debug_out.h"
  ...
  bool MarlinSettings::save() {
    ...
    //
    {
      ...
      #if HAS_BED_PROBE
        if (probe.offset.z > 10 || probe.offset.z < -10) probe.offset.z = 0;
    ...
    //
	  ...
	  // save quiet mode
	  EEPROM_WRITE(wtvar_quiet_mode);
	  
	  // save auto power off enable
      EEPROM_WRITE(wtvar_enablepowerloss);
  ...
  bool MarlinSettings::_load() {
    ...
    if (strncmp(version, stored_ver, 3) != 0) {
      ...
      DEBUG_ECHOLNPAIR("EEPROM version mismatch (EEPROM=", stored_ver, " Marlin=" EEPROM_VERSION ")");
        ...
        lcd_setstatus_P(GET_TEXT(MSG_ERR_EEPROM_VERSION));
      #endif
      eeprom_error = true;
    }
    else {
	...
	EEPROM_READ(wtvar_quiet_mode);
	if (wtvar_quiet_mode != 1)
		wtvar_quiet_mode = 0;
		
	EEPROM_READ(wtvar_enablepowerloss);
      if (wtvar_enablepowerloss > 1)
      ...
      if (eeprom_error) {
        ...
        #if HAS_LCD_MENU && DISABLED(EEPROM_AUTO_INIT)
          lcd_setstatus_P(GET_TEXT(MSG_ERR_EEPROM_INDEX));
        #endif
      }
        ...
        #if HAS_LCD_MENU && DISABLED(EEPROM_AUTO_INIT)
          lcd_setstatus_P(GET_TEXT(MSG_ERR_EEPROM_CRC));
        #endif
      }
      else if (!validating) {
        DEBUG_ECHO_START();
...
void MarlinSettings::reset() {
  ...
  //
    ...
    static float z_backup = probe.offset.z;
    if (z_backup > 10 || z_backup < -10) z_backup = 0;
  ...
  void MarlinSettings::report(const bool forReplay) {
     ...
     */
      ...
      LOOP_L_N(i, COUNT(lcd_preheat_hotend_temp)) {
        CONFIG_ECHO_START();
        SERIAL_ECHOLNPAIR(
            "  M145 S", (int)i
          , " H", TEMP_UNIT(lcd_preheat_hotend_temp[i])
          , " B", TEMP_UNIT(lcd_preheat_bed_temp[i])
          , " F", int(lcd_preheat_fan_speed[i])
        );
      }
